...
public class DownloadManager {
    ...
    private static ThreadPoolExecutor sThreadPool = new ThreadPoolExecutor(
        ...
        public Thread newThread(Runnable runnable) {
            Thread thread = new Thread(runnable, "download thread #" + mInteger.getAndIncrement()); //名字= "download thread #" + id
            return thread;
        }
    });
    ...
    public void download(final String url, final DownloadCallback callback) {
        ...
        if (mCache == null || mCache.size() == 0) { //数据库中没有数据
    ...
    private void processDownload(String url, long length, DownloadCallback callback, List<DownloadEntity> cache) {
        ...
        for (int i = 0; i < MAX_THREAD; i++) { //多个线程下载
